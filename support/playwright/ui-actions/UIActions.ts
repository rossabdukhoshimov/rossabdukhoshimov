...
export default class UIActions {
  ...
  public async goBack(description: string) {
    ...
    await this.page.goBack({ timeout: 30000, waitUntil: "load" });
  }
  ...
  public async goForward(description: string) {
    ...
    await this.page.goForward({ timeout: 30000, waitUntil: "load" });
  }
  ...
  public async pageRefresh() {
    ...
    await this.page.reload({ timeout: 30000, waitUntil: "load" });
  }
  ...
  /**
   * Gets the page cookies and saves them in a json file
   */
  public async getPageCookies(loggedUser: string): Promise<void> {
    await this.page.context().storageState({path: `../web/cookies/${loggedUser}.json`});
    Log.info(`Getting Page Cookies from the session of user: ${loggedUser}`)
  }


  /**
   * Loads previously saved page cookies to the browser
   */
  public async loadPageCookies(cookies: Array<any>): Promise<void> {
